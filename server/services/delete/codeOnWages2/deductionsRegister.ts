 ...
 *
 * Generates Form II — Register of Deductions.
...
import { FORM_II_MAPPING as M } from './mapping.js';
import {
  loadTemplate,
...
function buildRow(emp: MasterRow): Record<number, ExcelJS.CellValue> {
  const amountOrdered = getNumber(emp, M[12]);
  const amountRecovered = getNumber(emp, M[14]);
  const balance = amountOrdered !== null && amountRecovered !== null
    ? round2(amountOrdered - amountRecovered)
    : null;

  const grossWage = getNumber(emp, 101);
  const cumulativeDeducted = getNumber(emp, 125);
  ...
  return {
    ...
    12: roundOrBlank(amountOrdered),
    13: getValue(emp, M[13]),
    14: roundOrBlank(amountRecovered),
    15: balance,
    16: roundOrBlank(cumulativeDeducted),
    17: pctDeducted === null ? null : `${pctDeducted}%`,
    18: getValue(emp, M[18]),
  };
}
